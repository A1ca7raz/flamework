{ util, lib, path, ... }:
let
  wc = util.wc "plasma-org.kde.plasma.desktop-appletsrc";
  wcl = util.wc_ "$HOME/.config/plasma-org.kde.plasma.desktop-appletsrc";
...
in
{
  home.activation.setupDesktop = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    ${util.wc "kwinrc" "ModifierOnlyShortcuts" "Meta" "org.kde.lattedock,/Latte,org.kde.LatteDock,activateLauncherMenu"}
  '';
}