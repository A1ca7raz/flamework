...
with lib; let
  ...
  in rec {
    ${name} = let
      ...
      trivial = recursiveUpdate blank_trivial tpl_content;
    in {
      groups = {
      ...
      let
        ...
        content_from_tpl = recursiveUpdate trivial c;

        base = {
          components = forEach (util.try_list (_merge "components.use" [])) (x: components.${x});
          modules = forEach (util.try_list (_merge "modules.use" [])) (x: modules.${x});
      ...
      in recursiveUpdate content_from_tpl {
        ...
        modules.users = recursiveUpdate content_from_tpl.modules.users content.userModules;
      };
    };
  };
in
  fold (x: y: (mkTplWrapper x) // y) {} tpl_list